...
let () =
  ...
  match !action with
    Ast -> print_string "success" (* (Ast.string_of_program ast) *)
  | _ -> let sast = Semant.check ast in
    match !action with
      Ast     -> print_string "success\n" (* () *)
    | Sast    -> print_string "success\n" (* (Sast.string_of_sprogram sast) *)
    | LLVM_IR -> print_string (Llvm.string_of_llmodule (Codegen.trans sast)) (* print_string (Llvm.string_of_llmodule (Codegen.translate sast)) *)
    | Compile -> let m = Codegen.trans sast in
